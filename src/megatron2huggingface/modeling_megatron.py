"""
Megatron GPT model implementation for HuggingFace Transformers.

This module provides a HuggingFace-compatible implementation of Megatron-LM GPT models,
removing parallelism structures for single-device inference.
"""
...
class MegatronRotaryEmbedding(nn.Module):
    """Rotary Position Embedding implementation compatible with Megatron-LM."""
...
class MegatronDecoderLayer(nn.Module):
    """Decoder layer implementation compatible with Megatron-LM."""
...
class MegatronPreTrainedModel(PreTrainedModel):
    """Base class for Megatron models."""

    config_class = MegatronConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MegatronDecoderLayer"]
...
class MegatronModel(MegatronPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`MegatronDecoderLayer`]
    ...
    def __init__(self, config: MegatronConfig):
        ...
        if config.add_position_embedding and not config.use_rotary_position_embeddings:
            self.embed_positions = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        else:
            self.embed_positions = None

        self.layers = nn.ModuleList([MegatronDecoderLayer(config, layer_idx) for layer_idx in range(config.num_layers)])
        ...
        if attention_mask is not None and self._attn_implementation == "flash_attention_2" and use_cache:
            ...
            if is_padding_right:
                raise ValueError(
                    ...
                    " this may lead to unexpected behaviour for Flash Attention version of Megatron. Make sure to "
...
class MegatronForCausalLM(MegatronPreTrainedModel):
    """Megatron Model transformer with a language modeling head on top (linear layer with weights tied to the input embeddings)."""
    ...
    def __init__(self, config):
        ...
        if config.untie_embeddings_and_output_weights:
            ...
            # Tied weights - lm_head will use embed_tokens.weight
            self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
